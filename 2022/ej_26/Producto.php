...
class Producto
{
	...
	{
		$listaProductos = Producto::CargarProductosJson("productos");
		if($listaProductos)
		{
			$index = $this->ProductoExistente($listaProductos);

			if($index >=0 && $listaProductos[$index]->_stock >= $cantidad)
			{
				$listaProductos[$index]->_stock -= $cantidad;

				$this->_stock = $cantidad;
				Producto::GuardarJson($listaProductos);
				return 1;
			}
		}
		return 0;
	...
	static function BuscarProductoPorCodigo($codigoDeBarra)
	{
		...
		if(!is_null($lista))
		{
			...
			{
				if(!strcmp($prod->_codigoDeBarra, $codigoDeBarra))
				{
					return $prod;
				}
			}
